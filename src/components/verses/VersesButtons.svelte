<script>
	...
	const buttonClasses = `inline-flex items-center justify-center w-10 h-10 text-${window.colors.textNormal} transition-colors duration-150 rounded-3xl focus:shadow-outline bg-${window.colors.primary150} hover:bg-${window.colors.primary200} print:hidden`;
...
<div class="verseButtons flex flex-row space-x-2 text-{window.colors.textNormal} text-xs">
	...
	<div class="relative inline-block text-left" data-html2canvas-ignore>
		...
		<div class="{verseDropdownVisible === true ? 'block' : 'hidden'} verse-dropdown absolute z-10 mt-2 min-w-max origin-top-right bg-{window.colors.backgroundMain} divide-y divide-gray-{window.colors.textNormal} rounded-3xl shadow w-44 dark:bg-{window.colors.primary200}">
			<div class="py-2 space-y-4">
				<ul class=" text-sm text-{window.colors.primary500}" aria-labelledby="dropdownDefaultButton">
					<!-- play verse button -->
					<li>
						<button on:click={() => showAudioModal(key)} class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
					...
					<li>
						<button on:click={() => updateSettings({ type: 'userBookmarks', key })} class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
							<div class="flex flex-row items-center">
								<svelte:component this={userBookmarks.includes(key) ? Bookmarked : Bookmark} />
					...
					<!-- verse notes button -->
					<li>
						<button on:click={() => showNotesModal()} class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
							<div class="flex flex-row items-center">
								<Notes />
					...
					<li>
						<Link to="/morphology/{chapter}:{verse}" class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
							<div class="flex flex-row items-center">
								<Book />
					...
					<!-- share verse button -->
					<li>
						<button on:click={() => shareVerse()} class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
							<div class="flex flex-row items-center">
								<ShareOutline />
					...
					<li>
						<button on:click={() => downloadVerseImage(key)} class="block px-4 py-2 w-full text-left hover:bg-{window.colors.primary200}">
							<div class="flex flex-row items-center">
								<Photo />
