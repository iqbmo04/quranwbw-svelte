	...
	<div class="relative w-full max-w-md max-h-full">
		...
		<div class="relative bg-{window.websiteColors[1].backgroundMain} rounded-3xl shadow daaark:bg-{window.websiteColors[1].websitePrimaryDarkColor}">
			...
			<div class="px-6 py-6 space-y-8 lg:px-8">
				<h3 id="notes-modal-title" class="mb-8 text-xl font-medium text-{window.websiteColors[1].textNormal} daaark:text-white">{quranMetaData[chapter].transliteration}, {$__verseKey}</h3>
				<div class="">
					<textarea
						id="notes-value"
						rows="8"
						value={verseNote}
						class="block p-2.5 w-full text-sm text-{window.websiteColors[1].textNormal} bg-{window.websiteColors[1].websitePrimaryNormalrkColor} rounded-3xl border bg-{window.websiteColors[1].hoverBorder} focus:ring-gray-{window.websiteColors[1].textNormal} focus:border-blue-500 daaark:bg-{window.websiteColors[1].webwebsitePrimaryNormalolor} daaark:bg-{window.websiteColors[1]
							.hoverBorder} daaark:placeholder-gray-{window.websiteColors[1].textNormal} gray-{window.websiteColors[1].textNormal}ext-white daaark:focus:ring-gray-{window.websiteColors[1].textNormal} daaark:focus:border-blue-500"
				...
				<button id="notes-submit-button" on:click={() => updateNotesHandler()} class="w-full mr-2 mt-6 {buttonElement}">
					<svg class="w-3 h-3 text-{window.websiteColors[1].textNormal}" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
