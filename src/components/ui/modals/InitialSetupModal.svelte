...
<div id="initial-setup-modal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full theme-grayscale">
	<div class="relative w-full max-w-2xl max-h-full sm:mx-auto h-[calc(100%-3.5rem)] min-h-[calc(100%-3.5rem)] flex items-center">
		<!-- Modal content -->
		<div class="max-h-full overflow-hidden flex flex-col bg-{window.colors.backgroundMain} rounded-3xl shadow dark:bg-{window.colors.primary200}">
			<!-- Modal header -->
			<div class="flex items-start justify-between p-4 border-b rounded-t dark:bg-{window.colors.primary400}">
				<h3 class="text-xl font-semibold text-{window.colors.textNormal} dark:text-slate-400">Initial Setup</h3>
				<button on:click={() => toggleModal('initial-setup-modal', 'hide')} type="button" class="text-{window.colors.textNormal} bg-transparent hover:bg-{window.colors.primary150} hover:text-{window.colors.textNormal} rounded-3xl text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-{window.colors.primary150} dark:hover:text-white">
			...
			<div class="p-6 space-y-6 overflow-y-auto">
				...
				<div class="flex flex-row space-x-6">
					<div class="flex flex-col">
						<label for="quran-font-list" class="block mb-2 text-sm font-medium text-{window.colors.textNormal}">Quran Font</label>
						<select
							id="quran-font-list"
							on:change={(event) => updateSettings({ type: 'wordType', value: +event.target.value })}
							bind:value={$__wordType}
							class="w-32 border bg-{window.colors.primary400} text-{window.colors.textNormal} rounded-3xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-{window.colors.primary150} dark:bg-{window.colors.primary400} dark:placeholder-gray-{window.websiteColors[1].textNormal} dark:text-slate-400 dark:focus:ring-blue-500 dark:focus:border-blue-500"
						>
					...
					<div class="flex flex-col">
						<label for="display-style-list" class="block mb-2 text-sm font-medium text-{window.colors.textNormal}">Display Style</label>
						<select
