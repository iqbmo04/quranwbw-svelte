	...
	<div class="relative w-full max-w-2xl max-h-full sm:mx-auto h-[calc(100%-3.5rem)] min-h-[calc(100%-3.5rem)] flex items-center">
		...
		<div class="max-h-full overflow-hidden flex flex-col bg-{window.websiteColors[1].backgroundMain} rounded-3xl shadow dark:bg-{window.websiteColors[1].websitePrimaryDarkColor}">
			...
			<div class="flex items-start justify-between p-4 border-b rounded-t dark:bg-{window.websiteColors[1].hoverBorder}">
				<h3 class="text-xl font-semibold text-{window.websiteColors[1].textNormal} theme-grayscale">Tajweed Rules</h3>
			...
			<div class="p-6 space-y-6 overflow-y-auto">
				...
				<div>
					<table class="w-full text-sm text-left rtl:text-right text-{window.websiteColors[1].textNormal} dark:text-{window.websiteColors[1].textNormal}">
						<thead class="text-xs text-{window.websiteColors[1].textNormal} uppercase bg-{window.websiteColors[1].websitePrimaryNormalrkColor} theme-grayscale">
							<tr>
						...
						<tbody>
							{#each Object.entries(tajweedRulings) as [id, ruling]}
								<tr class="bg-{window.websiteColors[1].backgroundMain} border-b dark:bg-{window.websiteColors[1].websitePrimaryNormalrkColor} dark:bg-{window.websiteColors[1].hoverBorder} hover:bg-{window.websiteColors[1].webwebsitePrimaryNormalolor} dark:hover:bg-{window.websiteColors[1].websitwebsitePrimaryNormalr}">
