...
<div class={$__currentPage === 'chapter' || $__currentPage === 'page' ? 'block' : 'hidden'}>
	<div class="{$__bottomNavbarVisible === true ? 'block' : 'hidden'} fixed z-20 w-full h-16 max-w-xs md:max-w-lg shadow-sm -translate-x-1/2 bg-{window.colors.backgroundMain} border text-{window.colors.primary500} rounded-full bottom-4 left-1/2">
		<div class="grid h-full max-w-lg grid-cols-5 mx-auto">
			...
			<!-- 2nd icon -->
			<button type="button" title="Change Display" on:click={() => updateSettings({ type: 'displayType', value: $__displayType === 5 ? 1 : $__displayType + 1 })} class="opacity-70 inline-flex flex-col items-center justify-center px-5 relative inline-flex items-center hover:bg-{window.colors.primary150} group {$__currentPage === 'page' && disabledElement}">
				<Eye />
			...
			<div class="flex items-center justify-center">
				<button type="button" title="Play/Pause" on:click={() => audioController()} class="inline-flex flex-col items-center justify-center w-10 h-10 font-medium bg-{window.colors.primary150} hover:bg-{window.colors.primary150} rounded-full group focus:ring-2 focus:ring-blue-300 focus:outline-none">
			...
			<!-- 4th icon -->
			<button type="button" title="Settings" data-drawer-target="settings-drawer" data-drawer-show="settings-drawer" data-drawer-placement="right" aria-controls="settings-drawer" class="opacity-70 inline-flex flex-col items-center justify-center px-5 hover:bg-{window.colors.primary150} group">
				<Settings />
				<span class="sr-only">Settings</span>
			</button>

			<!-- Next Chapter -->
			<Link to="/{nextNavigation}" title="Next Chapter" class="{nextNavigationDisabled === true && disabledElement} inline-flex flex-col items-center justify-center px-5 rounded-e-full hover:bg-{window.colors.primary150} group">
				<ChevronRight />
