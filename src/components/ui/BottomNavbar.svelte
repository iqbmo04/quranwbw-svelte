...
<div class={$__currentPage === 'chapter' || $__currentPage === 'page' ? 'block' : 'hidden'}>
	<div class="{$__bottomNavbarVisible === true ? 'block' : 'hidden'} fixed z-20 w-full h-16 max-w-xs md:max-w-lg shadow-sm -translate-x-1/2 bg-{window.websiteColors[1].backgroundMain} border bg-{window.websiteColors[1].hoverBorder} rounded-full bottom-4 left-1/2 theme-grayscale">
		<div class="grid h-full max-w-lg grid-cols-5 mx-auto">
			...
			<div class="flex items-center justify-center">
				<button type="button" title="Play/Pause" on:click={() => audioController()} class="inline-flex flex-col items-center justify-center w-10 h-10 font-medium bg-{window.websiteColors[1].buttonBackground} hover:bg-{window.websiteColors[1].buttonBackgroundHoverColor} rounded-full group focus:ring-2 focus:ring-blue-300 focus:outline-none dark:focus:ring-blue-800">
